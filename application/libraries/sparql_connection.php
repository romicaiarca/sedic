...
function _sparql_a_connection($db)
{
    ...
    if ( !isset($db) )
    {
        ...
        {
            log_message('error', 'No currect SPARQL connection (connection) in play!');
            return FALSE;
        }
...
class sparql_connection {
    ...
    function __construct($endpoints)
    {
        if(is_array($endpoints))
        {
            ...
            {
                log_message('error', 'Incorect parameter!');
                return FALSE;
            }
    ...
    {
        $output = $this->dispatchQuery( "SELECT ?s ?p ?o WHERE { ?s ?p ?o } LIMIT 1" );
    ...
    {
        $output = $this->dispatchQuery( "SELECT (1+2 AS ?bar) WHERE { ?s ?p ?o } LIMIT 1" );
        return !isset($this->errno);
    }
    ...
    {
        $output = $this->dispatchQuery( "SELECT (\"foo\" AS ?bar) WHERE { ?s ?p ?o } LIMIT 1" );
        return !isset($this->errno);
    }
    ...
    function test_count()
    {
        ...
        $output = $this->dispatchQuery( "SELECT (COUNT(?p) AS ?n) ?o WHERE { <$s> ?p ?o } GROUP BY ?o" );
        return !isset($this->errno);
    }
    ...
    function test_max()
    {
        ...
        $output = $this->dispatchQuery( "SELECT (MAX(?p) AS ?max) ?o WHERE { <$s> ?p ?o } GROUP BY ?o") ;
        return !isset($this->errno);
    }
    ...
    function test_sample()
    {
        ...
        $output = $this->dispatchQuery( "SELECT (SAMPLE(?p) AS ?sam) ?o WHERE { <$s> ?p ?o } GROUP BY ?o" );
        return !isset($this->errno);
    }

    function test_load()
    {
        $output = $this->dispatchQuery( "LOAD <http://graphite.ecs.soton.ac.uk/sparqllib/examples/loadtest.rdf>" );
        return !isset($this->errno);
    }
}
